...
void printNode(struct Node *node, int numofdimensions, int depth) // prints the contents of a given node
{
    ...
    printf("[%d] ", depth); // print depth of the node
    if (is_leaf(node))      // checks if the given node is a leaf node.
    {
        printf("Leaf Node: ");
        for (int i = 0; i < node->num_of_children_or_tuples; i++) // iterates over the tuples in the node's list_of_tuples.
        {
            printTuple(node->list_of_tuples[i], numofdimensions); // prints each tuples
            printf(" ");
        }
        printf("\n");
    }
    else // If the node is not a leaf node, this else block is executed.
    {
        printf("Internal Node with %d children. Bounds: ", node->num_of_children_or_tuples); // prints the number of children in the internal node.
        printInternalNodeFromBounds(node->bounddefiners, numofdimensions);                   // prints the bounds of the internal node.
        for (int i = 0; i < node->num_of_children_or_tuples; i++)                            // iterates over the child nodes in the internal node's child_nodes array.
        {
            printNode(node->child_nodes[i], numofdimensions, depth + 1); // recursively calls the printNode by incrementing depth by 1.
        }
    }
}
